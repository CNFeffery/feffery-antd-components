...
export default class AntdTable extends Component {
    ...
    constructor(props) {
        ...
        this.onPageChange = (pagination, filter, sorter, currentData) => {
            ...
            if (currentData.action === 'paginate') {
                // 更新pageSize和currentData参数
                if (JSON.stringify(pagination) == '{}' || !pagination) {
                    // 当pagination为空对象，说明autoIgnorePagination被设置为true
                    // 且表中数据记录数量小于预设的pageSize
                    props.setProps({
                        pagination: false,
    ...
    render() {
        ...
        if (titlePopoverInfo) {
            for (let i = 0; i < columns.length; i++) {
                if (Object.keys(titlePopoverInfo).indexOf(columns[i].title) !== -1) {
                    let rawTitle = columns[i].title
                    let title = titlePopoverInfo[columns[i].title].title
                    let content = titlePopoverInfo[columns[i].title].content
                    columns[i]['title'] = () => {
        ...
        return (
            <ConfigProvider locale={zhCN}>
                <Table
                    ...
                    dataSource={data}
                    columns={columns}
                    pagination={data.length <= pagination.pageSize && autoIgnorePagination ? false : { ...pagination, ...{ showTotal: total => `${pagination.showTotalPrefix}${total}${pagination.showTotalSuffix}` } }}
                    bordered={bordered}
                    scroll={{
...
AntdTable.propTypes = {
    ...
    // 设置是否在数据记录行数少于pageSize时隐藏分页组件
    autoIgnorePagination: PropTypes.bool,
